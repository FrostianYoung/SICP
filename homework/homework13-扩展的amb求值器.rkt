  ...
  (let ((cprocs (map analyze (amb-choices exp))))
    (lambda (env succeed fail)
      (define (try-next choices)
        (if (null? choices)
            (fail)
            (let ((fail2 (lambda () (try-next (cdr choices)))))
              (set! last-fail fail2)
              ((car choices) env succeed fail2))))
      (try-next cprocs))))
...
(define (analyze-if-fail exp)
  (let ((first (analyze (cadr exp)))
    ...
    (lambda (env succeed fail)
      (let ((succeed2 (lambda (val _fail) (first env succeed fail)))
            (fail2 (lambda () (second env succeed fail))))
        (set! last-fail fail2)
        (first env succeed2 fail2)))))

(define (all-answer? exp) (tagged-list? exp 'all-answer))
(define (analyze-all-answer exp)
  (let ((exp (cadr exp)))
    (lambda (env succeed fail)
      (define (make-fail2 calls)
        (lambda ()
          (set! calls (+ calls 1))
          (if (= calls 1)
              (void)
              (displayln "There are no more answers."))))
      (let ((succeed2 (lambda (val _fail) (displayln val) (_fail)))
            (fail2 (make-fail2 0)))
        (set! last-fail fail2)
        ((analyze exp) env succeed2 fail2)))))
...
(define glb-fail
  (lambda () (displayln "There are no more answers.")))

(define glb-env (setup-environment))
(define last-fail glb-fail)
...
(define (my-driver-loop)
  (let ((input (read)))
    (cond ((eq? input eof) (void))
          ((eq? input `try-again) (begin (last-fail)
                                         (my-driver-loop)))
          (else (begin (ambeval input glb-env glb-succeed glb-fail)
                       (my-driver-loop))))))

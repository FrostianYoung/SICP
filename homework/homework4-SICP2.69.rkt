...
(define (successive-merge s)
  (define (operations current-set)
    (if (null? (cdr current-set))
        current-set
        (let ((left (car current-set))
              (right (cadr current-set)))
          (operations (cons (make-code-tree right left)
                            (cddr current-set))))))
  (car (operations s)))
...
(define (explode message)
  (compose1 (lambda (x) (map (compose1 string->symbol
                                       ~a)
                             x))
       string->list
       symbol->string))
...
(define (encode message tree)
  (define (operations message)
    (if (null? message)
        null
        (append (encode-symbol (car message) tree)
                (operations (cdr message)))))
  (operations (explode message)))
